...
# ── Path ─────────────────────────────────────────────────────────────────────
# Project root = parent of this db/ directory.
ROOT_DIR    = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DB_PATH     = os.environ.get("MEDISCAN_DB", os.path.join(ROOT_DIR, "mediscan.db"))

# ── Connection ────────────────────────────────────────────────────────────────
# Single connection, mutex-protected so multiple Flask threads share it safely.
_conn  = None
...
CREATE TABLE IF NOT EXISTS medicines (
    ...
    icon TEXT DEFAULT '💊', color TEXT DEFAULT 'teal', active INTEGER DEFAULT 1,
    notes TEXT DEFAULT '', pill_count INTEGER DEFAULT NULL,
    pills_per_dose INTEGER DEFAULT 1, refill_threshold INTEGER DEFAULT 7,
    created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS dose_logs (
    id TEXT PRIMARY KEY, medicine_id TEXT NOT NULL, scheduled TEXT NOT NULL,
    taken_at TEXT, taken INTEGER DEFAULT 0, date TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS fitness_activities (
    id TEXT PRIMARY KEY, name TEXT DEFAULT '', type TEXT NOT NULL,
    date TEXT NOT NULL, duration INTEGER DEFAULT 0, calories INTEGER DEFAULT 0,
    distance REAL DEFAULT 0, heart_rate INTEGER DEFAULT 0, notes TEXT DEFAULT '',
...
CREATE TABLE IF NOT EXISTS oauth_tokens (
    ...
    refresh_token TEXT DEFAULT '', expires_at TEXT DEFAULT '',
    scope TEXT DEFAULT '', created_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS user_profile (
    ...
    height_cm REAL DEFAULT 170, age INTEGER DEFAULT 30, sex TEXT DEFAULT 'male',
...
CREATE TABLE IF NOT EXISTS custom_foods (
    id TEXT PRIMARY KEY, name TEXT NOT NULL, calories REAL DEFAULT 0,
    protein REAL DEFAULT 0, carbs REAL DEFAULT 0, fat REAL DEFAULT 0,
    fiber REAL DEFAULT 0, serving_g REAL DEFAULT 100, created_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS vitals (
    id TEXT PRIMARY KEY, type TEXT NOT NULL, value1 REAL NOT NULL,
    value2 REAL DEFAULT NULL, unit TEXT DEFAULT '',
    date_key TEXT NOT NULL, logged_at TEXT NOT NULL
);
