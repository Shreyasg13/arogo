...
SEARCHABLE = [
    ...
    Spec('falls', 'Falls', '🩹',
         text=('what_happened', 'place', 'injury', 'notes'), date='fell_on',
         view='falls', show=('place', 'what_happened', 'injury', 'notes'),
         title=lambda r: r['place'] or 'Fall',
         meta=lambda r: _j(r['fell_on'], _clip(r['what_happened'], 60))),

    Spec('hearing_records', 'Hearing', '👂',
         text=('provider', 'finding', 'device', 'left_ear', 'right_ear', 'notes'),
         date='record_date', view='hearing',
         show=('kind', 'provider', 'device', 'finding', 'notes'),
         title=lambda r: r['device'] or r['provider'] or _cap(r['kind']),
         meta=lambda r: _j(r['record_date'], _clip(r['finding'], 60))),

    Spec('rehab_plans', 'Rehab plans', '🧘',
         text=('name', 'reason', 'prescribed_by', 'instructions'),
         date='started_on', view='rehab',
         show=('name', 'reason', 'prescribed_by', 'instructions'),
         title=lambda r: r['name'],
         meta=lambda r: _j(r['started_on'], _clip(r['reason'], 60))),
