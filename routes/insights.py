...
def api_health_score():
    """
    Compute today's 0-100 health score from 5 components (20 pts each):
    sleep · hydration · activity · nutrition · habits
    """
    import datetime as dt
    from db.wellness import get_sleep_logs, get_hydration_day
    from db.health   import get_habit_stats
    from db.fitness  import fitness_stats
    from db import get_nutrition_summary, get_profile, calc_tdee
    ...
    # ── 1. Sleep (last night's entry) ────────────────────────────
    sleep_score = 0
    sleep_detail = 'No sleep logged'
    sleep_logs   = get_sleep_logs(days=2)
    last_sleep   = next((s for s in reversed(sleep_logs)
                         if s['date_key'] in (today, yesterday)), None)
    if last_sleep:
        h = last_sleep['duration_h'] or 0
        q = last_sleep['quality'] or 3
        # Duration component: 0 at 0h, max at 8h
        dur_score  = min(h / 8.0, 1.0)
        # Quality component: 1-5 → 0-1
        qual_score = (q - 1) / 4.0
        sleep_score = round((dur_score * 0.7 + qual_score * 0.3) * 20)
        sleep_detail = f'{h}h · quality {q}/5'
    components.append({
        'key':    'sleep',
        'label':  'Sleep',
        'icon':   '🌙',
        ...
        'max':    20,
        'detail': sleep_detail,
        'tip':    'Aim for 7-9h with quality 4+' if sleep_score < 16 else 'Great sleep!',
    })

    # ── 2. Hydration (today) ──────────────────────────────────────
    hyd           = get_hydration_day(today)
    hyd_pct       = hyd.get('pct', 0)
    hyd_score     = round(min(hyd_pct / 100.0, 1.0) * 20)
    components.append({
        'key':    'hydration',
        'label':  'Hydration',
        'icon':   '💧',
        'score':  hyd_score,
        'max':    20,
        'detail': f'{hyd.get("total_ml", 0)}ml of {hyd.get("goal_ml", 2450)}ml goal',
        'tip':    'Drink more water' if hyd_score < 14 else 'Well hydrated!',
    })

    # ── 3. Activity (this week's minutes) ────────────────────────
    act_score   = 0
    ...
    components.append({
        ...
        'max':    20,
        'detail': act_detail,
        'tip':    f'{max(0, 150 - week_mins)} min to weekly goal' if act_score < 20 else '150+ min goal hit!',
    })

    # ── 4. Nutrition (today's calories vs target) ─────────────────
    nut_score   = 0
    ...
    if food_sum['log_count'] > 0:
        pct = eaten / target_cal if target_cal else 0
        # Ideal: 85–115% of target
        if 0.85 <= pct <= 1.15:
            nut_score = 20
        elif 0.70 <= pct < 0.85 or 1.15 < pct <= 1.30:
            nut_score = 14
        elif 0.50 <= pct < 0.70 or 1.30 < pct <= 1.50:
            nut_score = 8
        else:
            nut_score = 4
        nut_detail = f'{round(eaten)} of {round(target_cal)} kcal ({round(pct*100)}%)'
    components.append({
        'key':    'nutrition',
        'label':  'Nutrition',
        'icon':   '🍽️',
        ...
        'max':    20,
        'detail': nut_detail,
        'tip':    'Log meals for a full picture' if food_sum['log_count'] == 0 else
                  'On target!' if nut_score == 20 else 'Aim for 85-115% of calorie goal',
    })

    # ── 5. Habits (today's completion) ───────────────────────────
    hab_score  = 0
    hab_detail = 'No habits set up'
    hab_stats  = get_habit_stats(1)
    habits     = hab_stats.get('habits', [])
    if habits:
        done  = sum(1 for h in habits if h.get('done_today'))
        total = len(habits)
        hab_score  = round((done / total) * 20) if total else 0
        hab_detail = f'{done}/{total} habits done'
    components.append({
        'key':    'habits',
        'label':  'Habits',
        'icon':   '⭐',
        ...
        'max':    20,
        'detail': hab_detail,
        'tip':    'Keep building your routine!' if hab_score < 20 else 'Perfect habit day!',
    })
    ...
    elif total >= 65: grade, label = 'B', 'Good'
    elif total >= 50: grade, label = 'C', 'Fair'
    elif total >= 35: grade, label = 'D', 'Needs work'
    else:             grade, label = 'E', 'Just starting'

    # Dynamic message based on weakest component
